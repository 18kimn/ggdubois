...
furniture_spiral <- furniture %>%
  ...
  rowwise() %>%
  mutate(
    xend = min(houshold_value_dollars, max_x),
    yend = slope * xend + y
  ) %>%
  mutate(
    y_2 = yend,
...
furniture_label <- furniture_spiral %>%
  ...
  mutate(
    dollars = scales::dollar(houshold_value_dollars, prefix = "$"),
